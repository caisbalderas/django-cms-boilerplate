...
def generate_secret_key(file_path):
    ...
    length = 50
    SECRET_KEY = get_random_string(length, chars) # generates key if not already made

    with open('secret_key.py', 'w') as f:
        f.write("SECRET_KEY = '" + str(SECRET_KEY) + "'")
...
CMS_INSTALLED_APPS = (
    'suit', # Django Suit Admin Styling
    'cms',  # Django CMS
    ...
    # filer plugins, this project is configured for filer instead of the django-cms.file
    'filer',
...
CMS_TEMPLATE_CONTEXT_PROCESSORS = (
...
SECRET_KEY = 'qw%8r-p^5=y9k8wb3w369+)t_^(u#y6p$7*$l)bf^des!%p(i9'
...
) + TCP + CMS_TEMPLATE_CONTEXT_PROCESSORS
